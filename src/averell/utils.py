...
def generate_tei(corpora_list, output_path, ui_enabled=True):
    ...
    if ui_enabled:
        with ZipFile("corpora/TEI.zip", "w") as zfile:
            for corpus in corpora_list:
                p = Path(f'corpora/TEI/{corpus}')
                for f in p.glob("*/**/*.xml"):
                    zfile.write(f)
